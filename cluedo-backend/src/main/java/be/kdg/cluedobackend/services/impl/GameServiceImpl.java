...
public class GameServiceImpl implements GameService {
    ...
    public GameServiceImpl(CluedoRepository cluedoRepository, PlayerRepository playerRepository, UserRepository userRepository,
                           CardRepository cardRepository, RefreshGameMessageHandler refreshGameMessageHandler, SuggestionReplyCardHandler suggestionReplyCardHandler, SuggestionMessageHandler suggestionMessageHandler, SuggestionReplyHandler suggestionReplyHandler,
    ...
    public Map<SuggestionType, Player> makeSuggestion(int gameId, Scene scene, List<CardDto> suggestionCards) throws CluedoException {
        ...
        if (!playerFoundWithMatchingCards){
            ...
            this.suggestionReplyHandler.sendMessage(cluedo.getCluedoId(), new PlayerDto("", CharacterType.BLUE, 0), null);
        }
    ...
    private Map<SuggestionType, Player> loopOverEveryPlayer(Cluedo cluedo, List<Player> players, Scene scene) {
        ...
        for (int i = 1; i < players.size(); i++) {
            ...
            Player nextPlayer = players.stream().filter(p -> p.getGame_order() == indexToCheck).findFirst().get();
    ...
    private Map<SuggestionType, Player> sendWebsocketsIfPlayerHasAMatchingCard(Set<Card> matchingCards, Cluedo cluedo, Player nextPlayer, Map<SuggestionType, Player> exchangingPlayers) {
        if (matchingCards.size() > 0) {
            ...
            PlayerDto playerDto = new PlayerDto(nextPlayer.getUser().getUserName(), nextPlayer.getCharacterType(), nextPlayer.getPlayerId());
            this.suggestionReplyHandler.sendMessage(cluedo.getCluedoId(), playerDto, null);
    ...
    public boolean[] makeAccusation(int gameId, Scene scene, List<CardDto> suggestionCards) throws CluedoException {
        ...
        if (guess) {
            ...
            if (!hasEnded) {
                cluedo.switchTurn();
                this.refreshGameMessageHandler.sendMessage(cluedo.getCluedoId(), null, null);
            }else{
                cluedo.setWon(true);
            }
        }
    ...
    @Override
    public void replySuggestion(int cardId, int gameId, CardDto cardDto) throws CluedoException {
        Cluedo cluedo = getCluedoById(gameId);
        cluedo.getCurrentTurn().setShown(cardRepository.findById(cardId).get());
        cluedoRepository.save(cluedo);
